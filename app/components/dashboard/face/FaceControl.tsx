...
let optionsSSDMobileNet = undefined;

// helper function to pretty-print json object to string
function str(json) {
    let text = '<font color="blue">';
    text += json
        ? JSON.stringify(json)
              .replace(/{|}|"|\[|\]/g, '')
              .replace(/,/g, ', ')
...
function drawFaces(canvas: HTMLElement, data: string, fps) {
    //log(`Draw Faces: ${data.length} FPS: ${fps}`);
...
function detectVideo(video: HTMLVideoElement, canvas: HTMLElement) {
...
async function setupCamera() {
    ...
    log('Setting up camera', window.innerWidth, window.innerHeight);
    // setup webcam. note that navigator.mediaDevices requires that page is accessed via https
    if (!navigator.mediaDevices) {
        return null;
    }
    let stream: Promise<MediaStream> | MediaStream;
    const constraints = {
        audio: false,
        video: { facingMode: 'user', resizeMode: 'crop-and-scale', height: { ideal: 400 }, width: { ideal: 0 } },
    };
    if (window.innerWidth > window.innerHeight) constraints.video.width = { ideal: window.innerWidth };
    else constraints.video.height = { ideal: window.innerHeight };
    log(`Camera constraints: ${str(constraints)}`);
    try {
...
const FaceControl = () => {
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div id="log" style={{ overflowY: 'scroll', height: '200px' }}></div>
        </div>
    );
